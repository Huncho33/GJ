...
<body>
	<div id="myApply_wrapper">
		<div id="myApply_total">
			...
			<div id="myApply_tot">
				<div id="myApply_tit1">
					<h3 class="myApply_tit">월세지원 신청 현황</h3>
				</div>
				<div id="myApply_tit2">
					<p class="myApply_subtit" style="font-weight: bold;">
						<strong>｜</strong> 2023 청년월세지원 신청 현황
					</p>
				</div>
				<div id="myApply_cnt1">
					<table id="myApply_qnaList" align="center" width="100%" border=0>
						...
						<c:choose>
							<c:when test="${empty monthApplyList }">
								<tr height="35">
									<td colspan="5">
										<p align="center">
							...
							<c:when test="${not empty monthApplyList }">
								<c:forEach var="apply" items="${monthApplyList }">
									<tr align="center" height="35">
										<td width="25%">${apply.mo_policy}</td>
										<td width="40%">
										<c:choose>
											<c:when test="${not empty apply.mo_startpay}">
												${apply.mo_startpay} ~ ${apply.mo_endpay}
											</c:when>
											<c:otherwise>
												-
											</c:otherwise>
										...
										<td width="10%"><span style="color:blue;">${apply.mo_result}</span></td>
										<td width="15%">${apply.mo_date}</td>
										<td width="10%"><input type="hidden" name="mo_no" id="mo_no" value="${apply.mo_no }"><button type="button"
												onclick="location.href='${contextPath}/mypage/viewMonthApply.do?mo_no=${apply.mo_no }'"
