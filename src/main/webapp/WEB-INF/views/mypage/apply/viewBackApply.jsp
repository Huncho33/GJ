...
<c:set var="removeCompleted" value="${ViewBackMap.removeCompleted}" />
<c:set var="back" value="${ViewBackMap.applyBack}" />
<c:set var="backFileList" value="${ViewBackMap.backFileList}" />
<%
...
//우편번호 검색 기능
	...
	// 신청취소
	function fn_remove_apply(url,ba_no){
		var form = document.createElement("form");
        form.setAttribute("method", "post");
        form.setAttribute("action", url);
        var ba_noInput = document.createElement("input");
        ba_noInput.setAttribute("type","hidden");
        ba_noInput.setAttribute("name","ba_no");
        ba_noInput.setAttribute("value", ba_no);
        
        form.appendChild(ba_noInput);
	...
	function fn_modify_apply(form){
		form.action="${contextPath}/mypage/modBackApply.do";
		form.submit();
    }
					...
					<form name="applyFrm" id="applyFrm" method="post"
							...
							<table id="myApplyView_viewTable" align="center" width="100%"
								...
								<tbody>
									...
									<tr align="center">
										...
										<td width="25%"><a href="#">${back.ba_result}</a></td>
									</tr>
									<tr align="center">
									...
									<c:choose>
										<c:when
											test="${not empty backFileList && backFileList!='null' }">
											<c:forEach var="backFile" items="${backFileList}"
												varStatus="status">
												<tr id="tr_${status.count }" class="table_fileTr"
													...
													<td colspan="3" width="75%"><input type="hidden"
														name="oldFileName" value="${backFile.up_filename }" /> <input
						...
						<div class="myInfo_cnt2">
							<div id="tr_btn_modify">
								<div class="join_btn join_btn1">
									<input type="button" value="취소"
										onClick="location.href='${contextPath}/mypage/rentApplyList.do'">
								</div>
							...
							<div id="applyView_btn">
								<c:choose>
									<c:when test="${back.ba_result == '심사대기'}">
										<div class="join_btn join_btn3">
											<input type="button" value="신청취소"
												onClick="fn_remove_apply('${contextPath}/mypage/removeBackApply.do', ${back.ba_no})">
										</div>
										<div class="join_btn join_btn1">
								...
								<c:choose>
									<c:when test="${back.ba_result == '승인'}">
										<div class="join_btn join_btn2">
											<input type="button" value="변경신청" id="myInfo_mod"
												name="myInfo_mod" onClick="location.href='${contextPath}/mypage/modBackApplyView.do'">
										</div>
									</c:when>
								</c:choose>
							</div>
