...
<c:set var="removeCompleted" value="${ViewRentMap.removeCompleted}" />
<c:set var="rent" value="${ViewRentMap.applyRent}" />
<c:set var="rentFileList" value="${ViewRentMap.rentFileList}" />
<%
...
//우편번호 검색 기능
	...
	// 신청취소
	function fn_remove_apply(url,rent_no){
		var form = document.createElement("form");
        form.setAttribute("method", "post");
        form.setAttribute("action", url);
        var rent_noInput = document.createElement("input");
        rent_noInput.setAttribute("type","hidden");
        rent_noInput.setAttribute("name","rent_no");
        rent_noInput.setAttribute("value", rent_no);
        
        form.appendChild(rent_noInput);
	...
	function fn_modify_apply(form){
		form.action="${contextPath}/mypage/modRentApply.do";
		form.submit();
    }
					...
					<form name="applyFrm" id="applyFrm" method="post"
							...
							<table id="myApplyView_viewTable" align="center" width="100%"
								...
								<tbody>
									...
									<tr align="center">
										...
										<td width="25%"><a href="#">${rent.rent_result}</a></td>
									</tr>
									<tr align="center">
						...
						<div class="myInfo_cnt2">
							...
							<div id="applyView_btn">
								<c:choose>
									<c:when test="${rent.rent_result == '심사대기'}">
										<div class="join_btn join_btn3">
											<input type="button" value="신청취소"
												onClick="fn_remove_apply('${contextPath}/mypage/removeRentApply.do', ${rent.rent_no})">
										</div>
										<div class="join_btn join_btn1">
								...
								<c:choose>
									<c:when test="${rent.rent_result == '승인'}">
										<div class="join_btn join_btn2">
											<input type="button" value="변경신청" id="myInfo_mod"
												name="myInfo_mod" onClick="location.href='${contextPath}/mypage/modRentApplyView.do'">
										</div>
									</c:when>
								</c:choose>
							</div>
