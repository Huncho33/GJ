...
<c:set var="removeCompleted" value="${ViewRetMap.removeCompleted}" />
<c:set var="ret" value="${ViewRetMap.applyRentReturn}" />
<c:set var="retFileList" value="${ViewRetMap.retFileList}" />
<%
...
//우편번호 검색 기능
	...
	// 신청취소
	function fn_remove_apply(url,ret_no){
		var form = document.createElement("form");
        form.setAttribute("method", "post");
        form.setAttribute("action", url);
        var mo_noInput = document.createElement("input");
        mo_noInput.setAttribute("type","hidden");
        mo_noInput.setAttribute("name","ret_no");
        mo_noInput.setAttribute("value", ret_no);
	...
	function fn_modify_apply(form){
		form.action="${contextPath}/mypage/modRetApply.do";
		form.submit();
    }
					...
					<form name="applyFrm" id="applyFrm" method="post"
							...
							<table id="myApplyView_viewTable" align="center" width="100%"
								...
								<tbody>
									...
									<tr align="center">
										...
										<td width="25%"><a href="#">${ret.ret_result}</a></td>
									</tr>
									<tr align="center">
									...
									<c:choose>
										<c:when
											test="${not empty retFileList && retFileList!='null' }">
											<c:forEach var="retFile" items="${retFileList}"
												varStatus="status">
												<tr id="tr_${status.count }" class="table_fileTr"
													...
													<td colspan="3" width="75%"><input type="hidden"
														name="oldFileName" value="${retFile.up_filename }" /> <input
						...
						<div class="myInfo_cnt2">
							...
							<div id="applyView_btn">
								<c:choose>
									<c:when test="${ret.ret_result == '심사대기'}">
										<div class="join_btn join_btn3">
											<input type="button" value="신청취소"
												onClick="fn_remove_apply('${contextPath}/mypage/removeRetApply.do', ${ret.ret_no})">
										</div>
										<div class="join_btn join_btn1">
								...
								<c:choose>
									<c:when test="${ret.ret_result == '승인'}">
										<div class="join_btn join_btn2">
											<input type="button" value="변경신청" id="myInfo_mod"
												name="myInfo_mod" onClick="location.href='${contextPath}/mypage/modRetApplyView.do'">
										</div>
									</c:when>
								</c:choose>
							</div>
