...
<script>
 function fn_viewInfo(sh_no, member_id) {
	 action:
 }
</script>
</head>
		...
		<div id="adm_memberManage_contatiner">
			...
			<div id="adm_memberManage_tot">
				...
				<div id="adm_memberManage_search">
					...
					<form name="frmSearch"
						action="${contextPath}/admin/adminApply/adminSearchShareApply.do">
						...
						<option value="sh_result"
						<c:if test="${searchType eq 'sh_result' }">selected</c:if>>진행사항</option>
						</select>
					</form>
				</div>
					...
					<c:choose>
						...
						<c:when test="${not empty applyList }">
							<c:forEach var="join" items="${applyList}" varStatus="status" >
								<tr align="center">
									<td width="10%">
									 <a class='memberInfo' href="${contextPath}/admin/adminApply/adminShareViewApply.do?sh_no=${join.sh_no }&member_id=${join.member_id}">
										${join.sh_no}</a></td> 
									  <td width="15%">${join.member_id}</td>  
									<td width="10%">${join.membervo.member_name}</td>
									<td width="15%">${join.sh_policy}</td>
									<td width="12%">${join.sh_date}</td>
									<td width="15%">${join.sh_result}</td>
								</tr>
							</c:forEach>
						</c:when>
					</c:choose>
							...
							<c:when test="${totApply >100 }">
								...
								<c:forEach var="page" begin="1" end="10" step="1">
									<c:if test="${section >1 && page==1 }">
										<a class="no-uline"
											href="${contextPath}/admin/adminApply/adminShareApply.do?searchMember=${searchMember }&search=검+색section=${section-1}&pageNum=${(section-1)*10 +1 }">&nbsp;
											< </a>
									</c:if>
									<a class="no-uline"
										href="${contextPath }/admin/adminApply/adminShareApply.do?searchMember=${searchMember }&search=검+색section=${section}&pageNum=${page}">${(section-1)*10 +page }
									</a>
									<c:if test="${page ==10 }">
										<a class="no-uline"
											href="${contextPath }/admin/adminApply/adminShareApply.do?searchMember=${searchMember }&search=검+색section=${section+1}&pageNum=${section*10+1}">&nbsp;
											></a>
									</c:if>
								</c:forEach>
							</c:when>
								...
								<c:forEach var="page" begin="1" end="${totApply/10 +1}"
									...
									<c:choose>
										<c:when test="${page==pageNum }">
											<a class="sel-page"
												href="${contextPath }/admin/adminApply/adminShareApply.do?searchMember=${searchMember }&search=검+색section=${section}&pageNum=${page}">${page }
											</a>
										</c:when>
										<c:otherwise>
											<a class="no-uline"
