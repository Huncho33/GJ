...
<c:set var="searchWord" value="${qnasMap.searchWord}" />
<c:set var="searchType_qna" value="${qnasMap.searchType_qna}" />
<c:set var="searchTotQnas" value="${qnasMap.searchTotQnas}" />
	...
	<div id="qna_bground">
		<div id="qna_cnt">
			...
			<div id="adm_qna_tot">
				...
				<div id="qna_smallNav">
					<p>
						<span>[총 게시물: ${searchTotQnas}건]</span> <span class="qna_subnavi">청년패키지
							> 상담게시판</span>
					</p>
				</div>
				<!-- 검색 창 -->
							...
							<c:when test="${totQnas >100 }">
								...
								<c:forEach var="page" begin="1" end="10" step="1">
									<c:if test="${section >1 && page==1 }">
										<a class="no-uline"
											href="${contextPath }/adminQna/searchBoardList.do?searchType_qna=${searchType_qna}&searchWord=${searchWord }&search=검+색section=${section-1}&pageNum=${(section-1)*10 +1 }">&nbsp;
											pre </a>
									</c:if>
									<a class="no-uline"
										href="${contextPath }/adminQna/searchBoardList.do?searchType_qna=${searchType_qna}&searchWord=${searchWord }&search=검+색section=${section}&pageNum=${page}">${(section-1)*10 +page }
									</a>
									<c:if test="${page ==10 }">
										<a class="no-uline"
								...
								<c:forEach var="page" begin="1" end="${totQnas/10 +1}" step="1">
									<c:choose>
										<c:when test="${page==pageNum }">
											<a class="sel-page"
												href="${contextPath }/adminQna/searchBoardList.do?searchType_qna=${searchType_qna}&searchWord=${searchWord }&search=검+색section=${section}&pageNum=${page}">${page }
											</a>
										</c:when>
										<c:otherwise>
											<a class="no-uline"
