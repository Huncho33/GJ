...
<div id="monthApply_bground">
	...
	<form id="applyCond_frm" name="applyCond_frm" method="get" action="${contextPath}/rent/rentApplyForm2.do" onsubmit="return formCheck(this)">
		<div id="monthApply_title">
			<p class="monthApply_tit" style="font-weight: bold;"><i class="fa-solid fa-pen"></i>&nbsp;월세지원 신청 자격 확인</p>
		</div>
		
	
		<span>대구광역시 청년 월세 지원은 만 19~34세 이하 부모님과 별도 거주 중인 무주택 청년인 지원자만 신청가능합니다.
		본인은 해당 조건에 포함되나요?(공고월 기준 만 나이)</span>
		<br> 
		<input type="radio" name="check1" value="1" onClick="div_OnOff(this.value,'checkcon')" />예
		<input type="radio" name="check1" value="2"
			onClick="div_OnOff(this.value,'checkcon')" />아니요
		<div id="checkcon" style="display: none; color: red; font-size: 12px;">
			신청 자격에 준하지 않습니다.</div>


		<br>
		<span>현재 신청일 기준 원가구 소득 기준중위소득 100%이하 및 청년독립가구 소득 기준중의소득 60%이하에 해당되시나요? </span><br> <input type="radio"
			name="check2" value="3" onClick="div_OnOff2(this.value,'checkcon2')" />예
		<input type="radio" name="check2" value="4"
			onClick="div_OnOff2(this.value,'checkcon2')" />아니요
		<div id="checkcon2"
		...
		<br>
		<span>현재 신청일 기준 청년독립가구가 중위소득 50% 이상의 경제활동 등으로 원가구와 분리되어 기초보장제도상 (별도)보장가구로 인정되는 경우 
		청년독립가구의 소득(60% 이하)에 해당되시나요? </span><br> <input type="radio"
			name="check3" value="5" onClick="div_OnOff3(this.value,'checkcon3')" />예
		<input type="radio" name="check3" value="6"
			onClick="div_OnOff3(this.value,'checkcon3')" />아니요
		<div id="checkcon3"
			style="display: none; color: red; font-size: 12px;">신청 자격에 준하지
			않습니다.</div>
		...
		<br>
		<span>현재 신청일 기준 임차보증금 5천만원 및 월세 60만원 이하 주택에 거주하시나요?</span><br> <input type="radio"
			name="check4" value="7" onClick="div_OnOff4(this.value,'checkcon4')" />예
		<input type="radio" name="check4" value="8"
			onClick="div_OnOff4(this.value,'checkcon4')" />아니요
		<div id="checkcon4"
			style="display: none; color: red; font-size: 12px;">신청 자격에 준하지
			않습니다.</div>
